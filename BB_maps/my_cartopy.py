...
def proj_HRRR():
    ...
    lccProjParams_HRRR = {
        "central_latitude": -38.5,  # same as lat_0
...
def extent_centermap(ax, center, size=(3, 3)):
    ...
    if type(center) == tuple:
        ...
        states = np.genfromtxt(
            "/uufs/chpc.utah.edu/common/home/u0553130/pyBKB_v3/BB_maps/data/states_latlon.csv",
            names=True,
            delimiter=",",
            dtype=None,
            encoding="UTF-8",
        ...
        if center.upper() in np.char.upper(states["State"]):
            ...
            if a != "ERROR":
                ...
                print(
                    'Input must be a (lat,lon) tuple, a state like "Utah", or a MesoWest ID'
                )
...
def add_NEXRAD(ax, DATE="mostRecent", version="n0q", zorder=10):
    ...
    if DATE == "mostRecent":
        ax.add_wms(
            wms="https://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/%s-t.cgi?"
            % version,
...
def load_states(projection, resolution="500k"):
    ...
    if platform.system() == "Linux":
        source = (
            "/uufs/chpc.utah.edu/common/home/u0553130/pyBKB_v3/BB_maps/shapefiles/cb_2017_us_state_%s/cb_2017_us_state_%s.shp"
            % (resolution, resolution)
        )
    else:
        source = (
            "C:\\Users\\blaylockbk\\OneDrive\\Documents\\pyBKB_v3\\BB_maps\\shapefiles\\cb_2017_us_state_%s\\cb_2017_us_state_%s.shp"
            % (resolution, resolution)
        )
    return cfeature.ShapelyFeature(
        Reader(source).geometries(), projection, facecolor="none"
    )
...
def load_counties(projection, resolution="500k"):
    ...
    if platform.system() == "Linux":
        source = (
            "/uufs/chpc.utah.edu/common/home/u0553130/pyBKB_v3/BB_maps/shapefiles/cb_2017_us_county_%s/cb_2017_us_county_%s.shp"
            % (resolution, resolution)
        )
    else:
        source = (
            "C:\\Users\\blaylockbk\\OneDrive\\Documents\\pyBKB_v3\\BB_maps\\shapefiles\\cb_2017_us_county_%s\\cb_2017_us_county_%s.shp"
            % (resolution, resolution)
        )
    return cfeature.ShapelyFeature(
        Reader(source).geometries(), projection, facecolor="none"
