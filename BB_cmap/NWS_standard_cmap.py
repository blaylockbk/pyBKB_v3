...
def cm_temp(return_units=False, verbose=False):
    """
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Temperature cmap units are {units}")

    return kwargs


def cm_dpt(return_units=False, verbose=False):
    """
    Range of values:
        Celsius: -18 to 27 C
    ...
    COLORS = []
    for i, n in enumerate(norm):
        COLORS.append((n, np.array(C[i]) / 255.0))
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Dew Point cmap units are {units}")

    return kwargs


def cm_rh(return_units=False, verbose=False):
    """
    Range of values:
        5 to 90 %
    """
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Relative Humidity cmap units are {units}")

    return kwargs


def cm_wind(return_units=False, verbose=False):
    """
    m/s: vmin=0, vmax=60
    (MPH: vmin=0, vmax=140)
    """
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Wind Speed cmap units are {units}")

    return kwargs


def cm_sky(return_units=False, verbose=False):
    """
    Range of Values:
        0 to 90 %
    """
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Sky cover cmap units are {units}")

    return kwargs


def cm_precip(return_units=False, verbose=False):
    """
    Range of values:
    ...
    a = [0, 0.01, 0.1, 0.25, 0.5, 1, 1.5, 2, 3, 4, 6, 8, 10, 15, 20, 30]
    ...
    if return_units:
        kwargs["units"] = units
    if verbose:
        print(f"Precipitation cmap units are {units}")
