...
def osg_data(
    runDATE, XY_point_pairs, variable="GUST:surface", fxx=range(19), percentile=95
):
    """
    ...
    for f in fxx:
        ...
        DIR = (
            "/uufs/chpc.utah.edu/common/home/horel-group8/blaylock/HRRR_OSG/hourly30/%s/"
            % var
        )
...
if __name__ == "__main__":
    ...
    forecasts = forecast_data(runDATE, point_pairs, variable=variable, fxx=[fxx])

    # Get OSG Climatology data for those same points
    osg_climo = osg_data(
        runDATE, point_pairs, variable=variable, fxx=[fxx], percentile=percentile
    )
    ...
    plt.sca(ax2)
    plt.scatter(osg_climo["lon"], osg_climo["lat"], c=osg_climo["f%02d" % fxx], vmin=0)
    plt.colorbar()
    plt.title("%sth Percentile" % percentile, loc="left", fontweight="semibold")
    plt.title(
        "Valid %s" % (runDATE + timedelta(hours=fxx)).strftime("%d-%b %H:%M"),
        loc="right",
    )
