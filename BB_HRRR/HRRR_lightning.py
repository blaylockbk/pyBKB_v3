...
def get_HRRR_lightning(DATE, fxx=1):
    ...
    H = get_hrrr_variable(DATE, 'LTNG:entire', fxx=fxx,
...
def accumulate_grid_chunks(DATES, date_idx_chunks, fxx=1):
    ...
    accum_chunk = np.sum([get_HRRR_lightning(D, fxx=fxx) for D in DATES[start:end]], axis=0)
    return accum_chunk
...
def accumulate_HRRR_lightning(sDATE, eDATE, fxx=1):
    ...
    chunk_sections = [[DATES, (chunks[i], chunks[i+1]), fxx] for i in range(len(chunks)-1)]
...
if __name__ == '__main__': 
    ...
    plt.title('Accumulated HRRR F%02d Lightning Threat3' % fxx, loc='left', fontweight='semibold')
