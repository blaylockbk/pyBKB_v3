...
"""
Loop: Plot mean HRRR model spread FOR A THRESHOLD VALUE for every hour of the
day between a range of dates.
"""
...
def make_plots(MAP, data_left, data_right, variable, sDATE, eDATE, hour, fxx, condition, save=True):
    plt.cla()
    plt.clf()
    ...
    plt.sca(ax1)
    MAP.pcolormesh(lon, lat, AVG_SPREAD,
    ...
    plt.title('HRRR %s %s%s %s' % (variable, condition['condition'], condition['threshold'], VARS[variable]['units']), fontweight='bold', loc='left')
    ...
    plt.sca(ax2)
    MAP.contourf(lon, lat, data_right,
                 cmap='jet',
                 levels=range(1, np.ma.max(data_right)+2, 5),
                 latlon=True)
    MAP.drawcoastlines(linewidth=.3, color='w')
    MAP.drawcountries(linewidth=.3, color='w')
    MAP.drawstates(linewidth=.3, color='w')

    plt.title('HRRR Sample Count', fontweight='bold', loc='left')
    plt.title('Start: %s\n End: %s' % (sDATE.strftime('%d %b %Y'),
                                       eDATE.strftime('%d %b %Y')), loc='right', fontsize=10)
    ...
    # Additional savefig label for conditional statement
    if condition['condition'] == '>=':
        condition_str = 'gte-%s' % (condition['threshold'])
    elif condition['condition'] == '>':
        condition_str = 'gt-%s' % (condition['threshold'])
    elif condition['condition'] == '<=':
        condition_str = 'lte-%s' % (condition['threshold'])
    elif condition['condition'] == '<':
        condition_str = 'lt-%s' % (condition['threshold'])

    SAVEDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/PhD/HRRR_Spread/Hourly_%s-%s/%s_%s/%s/' % (sDATE.strftime('%b%Y'), eDATE.strftime('%b%Y'), variable.split(':')[0], condition_str, domain)
...
#for variable in ['APCP:surface', 'GUST:surface', 'UGRD:10 m', 'VGRD:10 m']:
    ...
    for hour in range(24):
        ...
        AVG_SPREAD, SAMPLE_COUNT = mean_spread_threshold(DATES, variable=variable, fxx=fxx, condition=condition)

        # Generate Figures
        make_plots(m, AVG_SPREAD, SAMPLE_COUNT, variable, sDATE, eDATE, hour, fxx, condition, save=True)
        make_plots(mW, AVG_SPREAD, SAMPLE_COUNT, variable, sDATE, eDATE, hour, fxx, condition,save=True)
        make_plots(mU, AVG_SPREAD, SAMPLE_COUNT, variable, sDATE, eDATE, hour, fxx, condition,save=True)
