    ...
    for season in seasons.keys():
        ...
        for ax, v, label in zip(axes, [var_0309, var_0915, var_1521, var_2103], labels):
            ...
            mesh = m.pcolormesh(LAND['lon'], LAND['lat'], np.sqrt(np.mean(v, axis=0)),
                                vmin=VARS[variable]['vmin'], vmax=VARS[variable]['vmax'],
                                latlon=True)
            m.drawcoastlines()
            plt.title(label)
        ...
        cb.ax.set_xlabel(r'%s Mean Model Spread \n %s (%s)' % (season, VARS[variable]['label'], VARS[variable]['units']));
        ...
        #mean_spread = np.sqrt(np.mean(all_variances, axis=0))
        seasons[season]['%s mean domain spread' % variable] = [np.mean(np.sqrt(np.mean(i, axis=0))) for i in [var_0309, var_0915, var_1521, var_2103]]
        
        seasons[season]['%s mean domain spread LAND' % variable] = [np.mean(np.ma.array(np.sqrt(np.mean(i, axis=0)), mask=LAND['value']==0)) for i in [var_0309, var_0915, var_1521, var_2103]]
        
        seasons[season]['%s mean domain spread WATER' % variable] = [np.mean(np.ma.array(np.sqrt(np.mean(i, axis=0)), mask=LAND['value']==1)) for i in [var_0309, var_0915, var_1521, var_2103]]
