...
def make_plots(
    MAP, data_left, data_right, variable, sDATE, eDATE, hour, fxx, condition, save=True
):
    plt.cla()
    plt.clf()
    ...
    SAVEDIR = "/uufs/chpc.utah.edu/common/home/u0553130/public_html/PhD/HRRR_Spread/Hourly_%s-%s/%s_%s/%s/" % (
        sDATE.strftime("%b%Y"),
        eDATE.strftime("%b%Y"),
        variable.split(":")[0],
        condition_str,
...
for variable in ["GUST:surface"]:
    ...
    for hour in range(24):
        ...
        AVG_SPREAD, SAMPLE_COUNT = mean_spread_threshold(
            DATES, variable=variable, fxx=fxx, condition=condition
        )
