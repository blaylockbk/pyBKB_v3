...
if __name__ == '__main__':
    ...
    #for variable in ['TMP:2 m']:
        ...
        for H in range(0, 24):
            ...
            plt.savefig(SAVEDIR+'CONUS_HRRR_RMSD-%s_hours-%s' % (variable.replace(":", '-').replace(' ', '-'), '-'.join('%02d' % x for x in HOURS)))
            ...
            plt.savefig(SAVEDIR+'WEST_HRRR_RMSD-%s_hours-%s' % (variable.replace(":", '-').replace(' ', '-'), '-'.join('%02d' % x for x in HOURS)))
