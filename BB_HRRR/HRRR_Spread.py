...
def spread(validDATE, variable, fxx=range(0, 19), verbose=True):
    ...
    H = np.array(
        [
            get_hrrr_variable(z[0], variable, fxx=z[1], verbose=False, value_only=True)[
                "value"
            ]
            for z in run_fxx
        ]
...
def mean_spread_MP(inputs):
    ...
    if verbose:
        msg = "Progress: %02d%% (%s of %s) complete ----> Downloading %s, %s\r" % (
            (i[0] + 1) / i[1] * 100,
            i[0] + 1,
            i[1],
            D.strftime("%d %b %Y %H:%M UTC"),
            variable,
        )
...
def mean_spread(
    validDATES, variable="TMP:2 m", fxx=range(0, 19), verbose=True, reduce_CPUs=3
):
    """
    ...
    args = [
        [(i, len(validDATES)), D, variable, fxx, verbose]
        for i, D in enumerate(validDATES)
    ]
...
def mean_spread_threshold_MP(inputs):
    ...
    if condition["condition"] == ">":
        masked_var_thresh = np.ma.array(
            var, mask=np.max(HH, axis=0) < condition["threshold"]
        )
    ...
    # Thus, find the min value of all forecasts, and mask all points that are .GT. the threshold.
        ...
        masked_var_thresh = np.ma.array(
            var, mask=np.min(HH, axis=0) > condition["threshold"]
        )
    elif condition["condition"] == "<=":
        masked_var_thresh = np.ma.array(
            var, mask=np.min(HH, axis=0) >= condition["threshold"]
...
def mean_spread_threshold(
    ...
    args = [
        [(i, len(validDATES)), D, variable, fxx, condition, verbose]
        for i, D in enumerate(validDATES)
    ]
    cpus = multiprocessing.cpu_count() - reduce_CPUs
    P = multiprocessing.Pool(cpus)
...
if __name__ == "__main__":
    ...
    m.pcolormesh(lon, lat, AVG_SPREAD, cmap="magma", latlon=True, vmin=0)

    m.drawcoastlines(linewidth=0.2, color="lightgrey")
    m.drawcountries(linewidth=0.2, color="lightgrey")
    m.drawstates(linewidth=0.2, color="lightgrey")

    plt.title("HRRR Model Spread %s" % variable, fontweight="bold", loc="left")
    plt.title(
        "Start: %s\n End: %s"
        % (sDATE.strftime("%d %b %Y %H:%M UTC"), eDATE.strftime("%d %b %Y %H:%M UTC")),
        loc="right",
    )
    plt.title("Hour: %s\nFXX: %s" % (hour, fxx))
    cb = plt.colorbar(orientation="horizontal", pad=0.01, shrink=0.8)
    cb.set_label("Mean Model Spread")
    # plt.savefig('./figs/hourly_SPREAD/CONUS_%s_h%02d' % (variable.replace(':', '-').replace(' ', '-'), hour))

    plt.figure(2, figsize=(9, 9))
    mW.pcolormesh(lon, lat, AVG_SPREAD, cmap="magma", latlon=True, vmin=0)

    mW.drawcoastlines(linewidth=0.2, color="lightgrey")
    mW.drawcountries(linewidth=0.2, color="lightgrey")
    mW.drawstates(linewidth=0.2, color="lightgrey")
    ...
    mU.pcolormesh(lon, lat, AVG_SPREAD, cmap="magma", latlon=True, vmin=0)

    mU.drawcoastlines(linewidth=0.2, color="lightgrey")
    mU.drawcountries(linewidth=0.2, color="lightgrey")
    mU.drawstates(linewidth=0.2, color="lightgrey")
