...
def member_multipro(inputs):
    ...
    first = ndimage.generic_filter(H['value'], first_filter, footprint=radial_footprint(radius), extra_keywords={'threshold':threshold})
    second = ndimage.generic_filter(first, second_filter, footprint=radial_footprint(radius))
    #
    return second


def TLE(validDATE, threshold=35, variable='REFC:entire', radius=9, fxx=range(2,5), hours_span=0):
    '''
...
if __name__ == '__main__':
    ...
    tle = TLE(validDATE, threshold=threshold, variable='REFC:entire', radius=radius, fxx=fxx, hours_span=hours_span)

    # Get the values at the valid time
    H = get_hrrr_variable(validDATE, 'REFC:entire', fxx=0)
    ...
    cm = cm_prob()
    m.pcolormesh(tle['lon'], tle['lat'], tle['prob'],
                    cmap=cm['cmap'],
                    vmax=cm['vmax'],
                    vmin=cm['vmin'],
                    latlon=True)
    ...
    m.contour(H['lon'], H['lat'], H['value']>=35,
            levels=[1],
            linewidths=.8,
            colors='crimson',
            latlon=True)
    ...
    plt.xlabel('Probability Reflectivity > %s dBZ\nFxx: %s\nFilter Radius: %s km' % (threshold, ['F%02d' % f for f in fxx], radius*3))
    plt.show()
