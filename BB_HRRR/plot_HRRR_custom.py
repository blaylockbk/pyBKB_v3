...
sys.path.append(
    "/uufs/chpc.utah.edu/sys/pkg/python/2.7.3_rhel6/lib/python2.7/site-packages/"
)
...
def load_lats_lons(model):
    ...
    if model in ["hrrr", "hrrrX"]:
        latlonpath = (
            "/uufs/chpc.utah.edu/common/home/horel-group7/Pando/hrrr/HRRR_latlon.h5"
        )
        latlonh5 = h5py.File(latlonpath, "r")
        lats = latlonh5["latitude"][:]
        lons = latlonh5["longitude"][:]
    elif model == "hrrrak":
...
def draw_map_base(
    ...
    if dsize == "full":
        if model != "hrrrak":
            ...
            m = np.load(
                ...
                + map_res
                + ".npy"
            ).item()  # premade HRRR conus object, it is faster processing??
        else:
            barb_thin = 75
        ...
        if dsize == "small":
            plus_minus_latlon = 0.27  # +/- latlon box around center point
            ...
            alpha = 0.75  # Alpha (pcolormesh transparency)
        ...
        m = Basemap(
            ...
            llcrnrlon=lon - plus_minus_latlon,
            llcrnrlat=lat - plus_minus_latlon,
            urcrnrlon=lon + plus_minus_latlon,
            urcrnrlat=lat + plus_minus_latlon,
        )

        if background == "arcgis":
            m.arcgisimage(
                service="World_Shaded_Relief", xpixels=arcgis_res, verbose=False
            )
        elif background == "arcgisSat":
            m.arcgisimage(
                service="ESRI_Imagery_World_2D", xpixels=arcgis_res, verbose=False
            )
    ...
    if plot_title:
        ...
        plt.title(
            "Run: %s F%02d" % (RUNDATE.strftime("%Y-%m-%d %H:%M UTC"), fxx), loc="left"
        )
...
def draw_wind(
    ...
    if Fill:
        ...
        cb = plt.colorbar(
            orientation="horizontal", pad=pad, shrink=shrink, extend="max"
        )
    ...
    if Barbs or Quiver:
        ...
        if barb_thin < 20:
            subset = hrrr_subset(
                H_UV, half_box=half_box, lat=lat, lon=lon, verbose=False
            )
        else:
            subset = H_UV

        # Need to rotate vectors for map projection
        subset["UGRD"], subset["VGRD"] = m.rotate_vector(
            subset["UGRD"], subset["VGRD"], subset["lon"], subset["lat"]
        )
        ...
        if Barbs:
            m.barbs(
                subset["lon"][::thin, ::thin],
                subset["lat"][::thin, ::thin],
                subset["UGRD"][::thin, ::thin],
                subset["VGRD"][::thin, ::thin],
                zorder=10,
                length=5.5,
                color=color,
    ...
    if p95:
        DIR = (
            "/uufs/chpc.utah.edu/common/home/horel-group8/blaylock/HRRR_OSG/hourly30/UVGRD_%s/"
            % level.replace(" ", "_")
        )
        FILE = "OSG_HRRR_%s_m%02d_d%02d_h%02d_f00.h5" % (
            (
                "UVGRD_%s" % level.replace(" ", "_"),
                VALIDDATE.month,
                VALIDDATE.day,
                VALIDDATE.hour,
            )
        ...
        cb.set_label(
            r"%s Wind Speed exceeding 95th Percentile (m s$\mathregular{^{-1}}$)"
            % level
        )
    ...
    if Convergence or Vorticity:
        ...
        if Vorticity:
            ...
            vort[np.logical_and(vort < 0.05, vort > -0.05)] = np.ma.masked
        ...
        if Convergence:
            ...
            conv[np.logical_and(conv < 0.05, conv > -0.05)] = np.ma.masked
...
def draw_tmp_dpt(
    ...
    if Fill:
        ...
        cbT = plt.colorbar(
            orientation="horizontal", shrink=shrink, pad=pad, extend="both"
        )
        cbT.set_label(label)
    ...
    if p05p95:
        DIR = (
            "/uufs/chpc.utah.edu/common/home/horel-group8/blaylock/HRRR_OSG/hourly30/%s_%s/"
            % (VAR, level.replace(" ", "_"))
        )
        FILE = "OSG_HRRR_%s_%s_m%02d_d%02d_h%02d_f00.h5" % (
            VAR,
            level.replace(" ", "_"),
            VALIDDATE.month,
            VALIDDATE.day,
        ...
        masked = (
            H["value"] - p05
        )  # both these datasets are in Kelvin, but when we take the difference it is in Celsius
        masked = np.ma.array(masked)
        masked[masked > 0] = np.ma.masked

        mesh_depression = m.pcolormesh(
            lons, lats, masked, vmax=10, vmin=-10, latlon=True, zorder=3, cmap=cmapOSG
        )
        ...
        mesh_exceedance = m.pcolormesh(
            lons, lats, masked, vmax=10, vmin=-10, latlon=True, zorder=3, cmap=cmapOSG
        )

        cb = plt.colorbar(
            orientation="horizontal", pad=pad, shrink=shrink, extend="both"
        )
        cb.set_label(
            r"5$\mathregular{^{th}}$/95$\mathregular{^{th}}$ percentile Depression/Exceedance %s"
            % label
        )
...
def draw_pot(
    ...
    m.pcolormesh(lons, lats, H["value"], cmap="coolwarm", zorder=2, latlon=True)
...
def draw_rh(
    ...
    m.pcolormesh(
        lons, lats, H["value"], cmap="RdYlGn", vmin=0, vmax=100, zorder=2, latlon=True
    )
    cbT = plt.colorbar(orientation="horizontal", pad=pad, shrink=shrink)
...
def draw_vpd(
    ...
    if level == "2 m":
        ...
        H_RH = {
            "value": Tempdwpt_to_RH(H_TMP["value"] - 273.15, H_DPT["value"] - 273.15)
        }

    if Fill:
        H_VPD = {
            "value": vapor_pressure_deficit(H_TMP["value"] - 273.15, H_RH["value"])
        }
        m.pcolormesh(
            lons,
            lats,
...
def draw_mslp(
    ...
    if Fill:
        m.pcolormesh(
            lons, lats, H["value"] / 100.0, latlon=True, cmap="viridis", zorder=2
        )
        cb = plt.colorbar(orientation="horizontal", pad=pad, shrink=shrink)
...
def draw_redflag(
    ...
    if Fill:
        ...
        masked[masked == 0] = np.ma.masked
        m.pcolormesh(
            lons, lats, masked, cmap="YlOrRd_r", alpha=alpha, zorder=5, latlon=True
        )
    ...
    if Fill_Potential:
        cdict3 = {
            "red": ((0.0, 1.0, 1.0), (0.5, 0.5, 0.5), (0.5, 1.0, 1.0), (1.0, 0.4, 0.4)),
            "green": (
                (0.0, 1.0, 1.0),
                (0.5, 0.5, 0.5),
                (0.5, 0.4, 0.4),
        ...
        RED_FLAG = np.logical_and(H_rh["value"] < rf_RH, H_gust["value"] > rf_WIND)
        # Linear Equation
    ...
    plt.xlabel(
        r"Red Flag Criteria: Winds > %s m s$\mathregular{^{-1}}$ and RH < %s%%"
        % (SPEED, RH)
    )
...
def draw_variable(
    ...
    if masked:
        ...
        if VAR == "APCP":
            values[values < 0.25] = np.ma.masked
        elif VAR == "LTNG":
            values[values < 0.01] = np.ma.masked
        else:
...
if __name__ == "__main__":
    ...
    for VALIDDATE in DATES:
        ...
        m, alpha, half_box, barb_thin = draw_map_base(
            model, dsize, background, location, lat, lon, RUNDATE, VALIDDATE, fxx
        )
        ...
        # plt.show()
        SAVEDIR = (
            "/uufs/chpc.utah.edu/common/home/u0553130/public_html/PhD/HRRR/Events_Day/%s/"
            % EVENT
        )
