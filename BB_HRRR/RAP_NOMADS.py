...
"""
Get data from a RAP grib2 file on the NOMADS server.
Requires cURL, wgrib2, and pygrib

This is basically a crude copy of the HRRR_Pando.py functions.

NOTE: The RAP files archived at NCDC are not the full RAP domain, but is a
      smaller domain centered over the contiguous United States.

Contents:
    get_rap_variable()        - Returns dict of single HRRR variable.
"""
...
def get_rap_variable(DATE, variable,
                     fxx=0,
                     model='rap',
                     removeFile=True,
                     value_only=False,
                     ...
                     outDIR='./'):
    """
    Uses cURL to grab the requested variable from a RAP grib2 file in the
    NOMADS NCDC archive. Uses the the requested variable string to search the
    .inv file (not .idx file; different name, but same thing) and determine the
    byte range. When the byte range of a variable is known, cURL is capable of
    downloading a single variable from a larger GRIB2 file. This function
    packages the data in a dictionary.
    ...
    Input:
        ...
        variable   - A string describing the variable you are looking for in the
                     GRIB2 file. Refer to the .inv files. For example:
                        https://nomads.ncdc.noaa.gov/data/rap130/201905/20190501/rap_130_20190501_0000_000.inv
        ...
        fxx        - The forecast hour you desire. Default is the analysis hour,
                     or f00.
        model      - The model you want. Options include ['rap']
    ...
    if model not in ['rap']:
        raise ValueError("Requested model must be 'rap'")
    
    # Check that you requested the right forecasts available for the model
    if model == 'rap' and fxx not in range(22):
        raise ValueError("RAP: fxx must be between 0 and 21\nYou requested f%02d" % fxx)
    ...
    if verbose:
        ...
        print("---------------------------------------------------------------------------\n")
    # File Format: 'https://nomads.ncdc.noaa.gov/data/rap130/201905/20190501/rap_130_20190501_0000_000.grb2'
    grib2file = 'https://nomads.ncdc.noaa.gov/data/rap130/%s/%s/rap_130_%s_%03d.grb2' \
                 % (DATE.strftime('%Y%m'), DATE.strftime('%Y%m%d'), DATE.strftime('%Y%m%d_%H%M'), fxx)
    fileidx = grib2file[:-5]+'.inv'
