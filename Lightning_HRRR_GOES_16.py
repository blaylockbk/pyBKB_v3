...
DATES += [sDATE + timedelta(hours=h) for h in range((eDATE-sDATE).days*24 + int((eDATE-sDATE).seconds/60/60))]



for DATE in DATES:
    SAVEDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/PhD/HRRR_RMSE/RMSE_events/%s/' % DATE.strftime('%Y-%m-%d_%H%M')
    ...
    for variable in ['REFC:entire', 'TMP:2 m', 'WIND:10 m',
                     'LTNG:entire', 'HGT:500 mb', 'DPT:2 m',
                     'CAPE:surface', 'UGRD:10 m', 'VGRD:10 m']:
        ...
        plt.savefig(SAVEDIR+'GLM_ABI_RMSD-_%s' % (variable.replace(':', '-').replace(' ', '-')))
        plt.cla()
        plt.clf()
        ...
        plt.title('HRRR %s RMSD and F%02d\nvalid: %s' % (variable, fxx, H['valid'].strftime('%Y-%m-%d %H:%M')), loc='right')

        plt.savefig(SAVEDIR+'GLM_anlys_RMSD-_%s' % (variable.replace(':', '-').replace(' ', '-')))
        plt.cla()
        plt.clf()
