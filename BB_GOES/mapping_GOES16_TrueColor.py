...
# Location of Utah
l = {"latitude": 39.75, "longitude": -111.5}
...
mZ = Basemap(
    ...
    projection="cyl",
    area_thresh=50000,
    llcrnrlon=l["longitude"] - 3.25,
    llcrnrlat=l["latitude"] - 3.25,
    urcrnrlon=l["longitude"] + 3.25,
    urcrnrlat=l["latitude"] + 3.25,
)
...
RGB_IR = np.dstack(
    [np.maximum(R, cleanIR), np.maximum(G_true, cleanIR), np.maximum(B, cleanIR)]
)
...
plt.title(
    "GOES-16 True Color and Night IR", loc="left", fontweight="semibold", fontsize=15
)
plt.title("%s" % scan_start.strftime("%H:%M UTC %d %B %Y"), loc="right")
...
# Add in clean IR
RGB_contrast_IR = np.dstack(
    [
        np.maximum(RGB_contrast[:, :, 0], cleanIR),
